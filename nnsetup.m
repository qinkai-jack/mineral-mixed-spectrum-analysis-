...
% layers, architecture being a n x 1 vector of layer sizes e.g. [784 100 10]
    ...
    nn.learningRate                     = 0.00001;            %  learning rate Note: typically needs to be lower when using 'sigm' activation function and non-normalized inputs.
    nn.momentum                         = 0.9;          %  Momentum
    nn.scaling_learningRate             = 0.8;            %  Scaling factor for the learning rate (each epoch)
    ...
    for i = 2 : nn.n    %权值初始化
      ...
      %%
       ...
       % nn.W{i - 1} = (rand(nn.size(i), nn.size(i - 1)) - 0.5) * 2 * 4 * sqrt(6 / (nn.size(i) + nn.size(i - 1))/(1 + nn.ra(i-1)^2));
